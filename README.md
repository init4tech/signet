...
Each binary (`signet`, `signet-exex`, `signet-sidecar`) is versioned independently. A configuration change that affects only one binary results in only that binary's version being bumped.
